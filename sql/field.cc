...
{
  DBUG_ENTER("Interval_list::create_typelib");
  ...
  uint nbytes= (sizeof(char*) + sizeof(uint)) * (interval_list.elements + 1);
  if (!(interval->type_names= (const char **) alloc_root(mem_root, nbytes)))
    goto err; // EOM;
  interval->name= "";
  interval->count= interval_list.elements;
  interval->type_lengths= (uint*) (interval->type_names + interval->count + 1);
                                               ...
                                               Field *orig_field)
{
  ...
  interval_list.empty(); // prepare_interval_list() needs this
